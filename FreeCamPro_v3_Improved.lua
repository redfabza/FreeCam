-- FreeCamPro v3 Enhanced · WackShop
-- ✨ Fixed mobile + PC controls & added zoom system for both platforms
...
local function makeDraggable(element)
	...
	element.InputBegan:Connect(function(i)
		if isClickInput(i.UserInputType) then
			dragState.on = true
			dragState.s = i.Position
			dragState.o = element.Position
		end
	end)
	UIS.InputChanged:Connect(function(i)
		if dragState.on and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
			local d = i.Position - dragState.s
			element.Position = UDim2.new(dragState.o.X.Scale, dragState.o.X.Offset + d.X, dragState.o.Y.Scale, dragState.o.Y.Offset + d.Y)
		end
	end)
	UIS.InputEnded:Connect(function(i)
		if isClickInput(i.UserInputType) then dragState.on = false end
	end)
end
...
-- ===== ZOOM FUNCTION (Mouse wheel + mobile pinch) =====
...
UIS.InputChanged:Connect(function(input, gameProcessed)
	...
	if input.UserInputType == Enum.UserInputType.MouseWheel then
		local delta = -input.Position.Z * zoomSpeed
		updateZoom(delta)
	end
end)
...
UIS.InputBegan:Connect(function(input, gameProcessed)
	if gameProcessed or not camOn or not ready then return end
	
	if input.KeyCode == Enum.KeyCode.W then mv = mv + Vector2.new(0, 1) end
	if input.KeyCode == Enum.KeyCode.S then mv = mv + Vector2.new(0, -1) end
	if input.KeyCode == Enum.KeyCode.A then mv = mv + Vector2.new(-1, 0) end
	if input.KeyCode == Enum.KeyCode.D then mv = mv + Vector2.new(1, 0) end
	
	-- Zoom with Q/E
	if input.KeyCode == Enum.KeyCode.Q then updateZoom(-zoomSpeed) end
	if input.KeyCode == Enum.KeyCode.E then updateZoom(zoomSpeed) end
end)
...
UIS.InputChanged:Connect(function(input)
	if not camOn or not ready or not mouseDown then return end
	if input.UserInputType == Enum.UserInputType.MouseMovement then
		tRY = tRY - input.Delta.X * SENS
		tRX = math.clamp(tRX - input.Delta.Y * SENS, -85, 85)
	end
end)
...
panel.Size=UDim2.fromOffset(230,420)
panel.Position=UDim2.new(1,-248,0.5,-210)
...
mkLbl(title,"v3.2 · Ultra Smooth",Color3.fromRGB(70,88,130),10,Enum.Font.Gotham,Enum.TextXAlignment.Left,UDim2.new(1,-90,0,14),UDim2.new(0,50,0,28))
...
xBtn.InputEnded:Connect(function(i)
	if not isClickInput(i.UserInputType) then return end
	if camOn then
...
-- ===== NEW: Smoothness Control =====
local smoothLabel=Instance.new("TextLabel",panel)
smoothLabel.Size=UDim2.new(1,-16,0,14);smoothLabel.Position=UDim2.new(0,12,0,212);smoothLabel.BackgroundTransparency=1
smoothLabel.Text="💫 MOVEMENT SMOOTH";smoothLabel.TextColor3=Color3.fromRGB(60,80,120);smoothLabel.Font=Enum.Font.GothamBold
smoothLabel.TextSize=10;smoothLabel.TextXAlignment=Enum.TextXAlignment.Left

mkSlider("",228,SMOOTH,1,20,0.5,Color3.fromRGB(100,220,255),function(v)
	SMOOTH=v
end)

mkLine(266)

-- ===== ZOOM SLIDER (PC + Mobile) =====
local zoomLabel=Instance.new("TextLabel",panel)
zoomLabel.Size=UDim2.new(1,-16,0,14);zoomLabel.Position=UDim2.new(0,12,0,270);zoomLabel.BackgroundTransparency=1
zoomLabel.Text="🔍 ZOOM";zoomLabel.TextColor3=Color3.fromRGB(60,80,120);zoomLabel.Font=Enum.Font.GothamBold
zoomLabel.TextSize=10;zoomLabel.TextXAlignment=Enum.TextXAlignment.Left

mkSlider("",286,currentZoom,minZoom,maxZoom,1,Color3.fromRGB(200,100,255),function(v)
	currentZoom=v
	if camOn and ready then cam.FieldOfView=v end
end)

mkLine(324)

mkToggle("🔭  FOV Override","ปรับ Field of View",330,
	function() fovPanel.Visible=true end,
	function() fovPanel.Visible=false;cam.FieldOfView=70;currentZoom=70 end
)
...
local function fRef()
	...
	fBadge.Text=tostring(math.floor(fovV+0.5)).."°";cam.FieldOfView=fovV;currentZoom=fovV
end
...
fHit.InputBegan:Connect(function(i)
	if isClickInput(i.UserInputType) then fSld=true;fFromI(i) end
end)
UIS.InputChanged:Connect(function(i)
...
UIS.InputEnded:Connect(function(i)
	if isClickInput(i.UserInputType) then fSld=false end
end)
fPlus.InputEnded:Connect(function(i)
	if isClickInput(i.UserInputType) then fovV=math.min(120,fovV+5);fRef() end
end)
fMinus.InputEnded:Connect(function(i)
	if isClickInput(i.UserInputType) then fovV=math.max(10,fovV-5);fRef() end
end)

-- ===== W BUTTON (TOGGLE PANEL) =====
...
-- Mobile touch zoom detection
local lastTouchDistance = 0
local zoomTouchIds = {}

UIS.TouchStarted:Connect(function(inp,_)
	if not camOn then return end
	
	zoomTouchIds[inp] = true
	
	-- Left side = movement joystick
	if inp.Position.X < cam.ViewportSize.X/2 then
		if not jOn then jTid=inp;jStart(inp.Position) end
	-- Right side = rotation + zoom
	else
		if not rOn then rOn=true;rTid=inp end
	end
	
	-- For pinch zoom (2 fingers)
	if #zoomTouchIds == 2 then
		local touches = {}
		for t,_ in pairs(zoomTouchIds) do
			table.insert(touches, t)
		end
		if #touches == 2 then
			lastTouchDistance = (touches[1].Position - touches[2].Position).Magnitude
		end
	end
end)
...
UIS.TouchMoved:Connect(function(inp)
	...
	elseif inp==rTid and rOn then
		tRY=tRY-inp.Delta.X*SENS;tRX=math.clamp(tRX-inp.Delta.Y*SENS,-85,85)
	end
	...
	if #zoomTouchIds >= 2 then
		...
		if #touches == 2 then
			local newDist = (touches[1].Position - touches[2].Position).Magnitude
			local delta = newDist - lastTouchDistance
			if math.abs(delta) > 2 then
				updateZoom(delta * 0.1)
...
UIS.TouchEnded:Connect(function(inp)
	zoomTouchIds[inp] = nil
	if inp==jTid then jEnd() end
	if inp==rTid then rOn=false;rTid=nil end
	lastTouchDistance = 0
end)

print("✅ FreeCamPro v3.2 โหลดสำเร็จ - Ultra Smooth Edition")
print("⌨️  PC: WASD = เคลื่อนที่ | ขวาคลิก+ลาก = หมุน | Q/E = ซูม | Scroll = ซูม")
print("📱 Mobile: ซ้าย = เคลื่อนที่ | ขวา = หมุน | 2 นิ้ว pinch = ซูม")
print("💫 ใช้ MOVEMENT SMOOTH slider ในเมนูปรับความลื่นของการเคลื่อนที่")
